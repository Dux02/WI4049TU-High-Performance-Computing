...
double integrate_v2(int rank, int nproc, int local_n, double const* local_x, double const* local_fx)
{
    ...
    {   
        int reverse_rank = nproc - 1 - rank;

        if (reverse_rank % (2 * step) == 0 )
        {
    ...
    else if (reverse_rank % (2 * step) == step)
    {
...
double integrate_v3(
    ...
    double const *local_fx)
{
    /* Compute this rank's local contribution */
    double local_I = trapezoidal(local_n, local_x, local_fx);

    /* The complete integral will be stored here on the root */
    double global_I = 0.0;

    /* Sum all local integrals onto the final MPI process */
    MPI_Reduce(
        &local_I,
        &global_I,
        1,
...
/* Distribute intervals as evenly as possible */
int base_n = global_n / nproc;
int remainder = global_n % nproc;
...
/* Global interval at which this rank starts */
int start = rank * base_n + min(rank, remainder);
...
double a_local = a + start * h;
    ...
    t1 = run_benchmark(integrate_v1, num_runs, rank, nproc, local_n, local_x, local_fx);
    t2 = run_benchmark(
    integrate_v2,
    num_runs,
    rank,
...
t3 = run_benchmark(integrate_v3, num_runs, rank, nproc,
                   local_n, local_x, local_fx);
